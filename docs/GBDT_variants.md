...
> [!note]
> :question: This method only supports layer-wise tree construction. The algorithm scans through the entire dataset and collectively decides the split points for all leaves, but I am unsure about the details. Also, this approach doesn't support leave-wise tree construction, possibly due to this. 
...
> [!important]
> The quantile finding step is optimized to only take a linear scan over the feature, which is $O(n)$, given the block structure. 

> [!note]
> $O(nlog(\\# candidates))$ is taken in the aggregation step, where you assign the rest of the data into the bins through binary search. This is claimed to be further optimized into linear complexity $O(n)$ with the block unit and noval data structure. 
...
> [!note]
> The real implementation of XGBoost supports histogram-based proposal method as well. 
...
> [!note]
> SGD should be much faster than GOSS since don't need to sort.
...
> [!note]
> Set $\gamma$ carefully when the dataset is large, as $\gamma$ is actually a ratio. The `number` of conflicts are propotional to the loss in accuracy. 
...
> [!important]
> To achieve sparse-awared histogram building algorithm, zero values are ignored. 
